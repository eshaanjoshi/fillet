...
use crate::token_enums::{self, LiteralData};
...
fn binary(expr:Expr) -> LiteralData{
    if let Expr::Binary(left, op, right) = expr{
        ...
        let left_data = eval::evaluate(*left);
        let right_data = eval::evaluate(*right);
        match op.ty{
            BinaryOpTy::Minus => {
                match left_data {
                    LiteralData::NUM(_) => {
            ...
            BinaryOpTy::Plus => {
                match left_data {
                    ...
                    LiteralData::STR(_) => {
                        return strcat(left_data, right_data, |a:String, b:String| format!("{}{}", a, b))
                    }
                    _ => exit(1),
                }
            },
            BinaryOpTy::EqualEqual |BinaryOpTy::NotEqual | BinaryOpTy::Greater | BinaryOpTy::GreaterEqual | BinaryOpTy::Less | BinaryOpTy::LessEqual => return comp_op(left_data, right_data, op.ty),
...
fn unary(expr:Expr)->LiteralData{
    if let Expr::Unary(op, expr) = expr{
        ...
        match op.ty{
            ...
            UnaryOpTy::Bang => {
                match val{
                    LiteralData::BOOL(f) => return LiteralData::BOOL(truthy(val)),
                    _=> exit(1),
                }
            },
        }
...
pub fn accept(expr:Expr)->LiteralData{// -> impl Fn(Expr) -> token_enums::LiteralData{
    match expr {
        Expr::Binary(_,_,_) => return binary(expr),
        Expr::Unary(_,_) => return unary(expr),
        Expr::Literal(_) => return eval::literal(expr),
        Expr::Grouping(_) => return eval::grouping(expr),
        _=>exit(1),
    }
}
